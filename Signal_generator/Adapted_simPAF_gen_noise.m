function multileadNoise = Adapted_simPAF_gen_noise(ecgLength, noiseType, noiseRMS, DATAnoises)
%
...
switch noiseType 
    ...
    case 1     % motion artefacts
       ...
       if ecgLength < noiseLength
          multileadNoise = DATAnoises.motion_artefacts(:, noiseStart+1:noiseStart+ecgLength);
       else
          multileadNoise = DATAnoises.motion_artefacts(:, noiseStart+1:noiseStart+(noiseLength/2));
          for i = 1:cycles
    ...
    case 2     % electrode movement
       %DATAnoises = load('DATA_noises_real'); 
       if ecgLength < noiseLength
          multileadNoise = DATAnoises.electrode_movement(:, noiseStart+1:noiseStart+ecgLength);
       else
          multileadNoise = DATAnoises.electrode_movement(:, noiseStart+1:noiseStart+(noiseLength/2));
          for i = 1:cycles
              noiseTemp = [noiseTemp multileadNoise]; 
          end
          multileadNoise = noiseTemp(:,1:ecgLength);
    ...
    case 3     % baseline wander
       %DATAnoises = load('DATA_noises_real'); 
       if ecgLength < noiseLength
          multileadNoise = DATAnoises.baseline_wander(:, noiseStart+1:noiseStart+ecgLength);
       else
          multileadNoise = DATAnoises.baseline_wander(:, noiseStart+1:noiseStart+(noiseLength/2));
          for i = 1:cycles
              noiseTemp = [noiseTemp multileadNoise]; 
          end
          multileadNoise = noiseTemp(:,1:ecgLength);
    ...
    case 4     % mixture of noises
       %DATAnoises = load('DATA_noises_real');
       if ecgLength < noiseLength
          multileadNoise = DATAnoises.mixture_of_noises(:, noiseStart+1:noiseStart+ecgLength);
       else
          multileadNoise = DATAnoises.mixture_of_noises(:, noiseStart+1:noiseStart+(noiseLength/2));
          cycles
          for i = 1:cycles
              noiseTemp = [noiseTemp multileadNoise]; 
          end
